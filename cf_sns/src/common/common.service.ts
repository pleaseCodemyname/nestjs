...
export class CommonService {
  ...
  private composeFindOptions<T extends BaseModel>(
    ...
    /**
     * where, order, take, skip(페이지 기반일때만)
     */
    /**
     ...
     * }
     *
     * 현재는 where__id_more_than / where__id__less_than에 해돵되는 where 필터만 사용중이지만
     * 나중에 where__likeCount__more_than 이나 where__title_ilike 등 추가 필터를 넣고 싶어졌을 때
     ...
     *      ['where', 'id', 'more_than']
     * 3-2) 2개의 값으로 나뉜다면, 정확한 값을 필터하는 것이기 때문에 operator없이 적용한다.
     *      where__id (2개로만 split) --> ['where', 'id'] 유틸리티 적용할 필요 없음
     * 4) order의 경우 3-2와 같이 적용(order는 항상 2개로만 적용)
     */
